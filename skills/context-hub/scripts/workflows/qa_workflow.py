...
def _require_existing_capability(capability_dir: Path, capability_name: str) -> None:
    if capability_dir.exists():
        return
    raise ValueError(f"qa workflow 需已有 capability 或 PM 先建: {capability_name}")
...
def run_qa_workflow(
    *,
    ...
    target_file = capability_target_document_path(capability_dir, "qa")
    ...
    testing_sources_path = root / "topology" / "testing-sources.yaml"
    ...
    if request["action"] == "align" and resolved_task_ref:
        try:
            ...
            warnings.append("未实时校验 ONES 测试任务，已回退到 hub 内 testing sources")
            if testing_sources_path.exists():
                used_sources.append(testing_sources_path)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="运行 QA workflow skeleton")
...
def main() -> int:
    ...
    if args.output_format == "json":
        ...
        print(f"QA workflow complete: {result['capability']} -> {result['target_file']}")
    return 0
